...
def get_place_input():
    ...
    if len(parts) == 2 and len(parts[1]) > 2:
        country_lookup = {
            ...
            "spain": "ES", "germany": "DE", "italy": "IT", "kenya": "KE"
        }
...
def get_weather_data(lat, lon, date_obj):
    ...
    if response.status_code == 200:
        ...
        st.warning(f"No data for {date_obj.strftime('%Y-%m-%d')} (status {response.status_code})")
        return None
...
def safe_lineplot(ax, dates, temps, label, color):
    """Plot only if data exists and lengths match."""
    if temps and len(dates) == len(temps):
        sns.lineplot(
            x=dates, y=temps,
            marker='o', linewidth=2, label=label, color=color, ax=ax
        )
    else:
        st.info(f"Skipping {label} — no valid data.")
...
if place_data and start_date and st.button("Get Weather Data"):
    ...
    if not any([current_f, one_year_f, ten_years_f, eighty_f]):
        st.error("No weather data available for this place and date.")
        st.stop()
    ...
    safe_lineplot(ax, dates, current_f, f'{dates[0].year}', palette[0])
    safe_lineplot(ax, dates, one_year_f, f'{dates[0].year-1}', palette[1])
    safe_lineplot(ax, dates, ten_years_f, f'{dates[0].year-10}', palette[2])
    safe_lineplot(ax, dates, eighty_f, '1979', palette[3])
